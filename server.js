...
app.get("/",(req,res)=>res.json({status:"ok",message:"KIDA 可菱水提醒系統運作中"}));
...
async function runFilterReminders(){
  try{const rows=await dbGet("reminders","?notified=eq.0");let sent=0;
    for(const r of rows){const d=daysDiff(r.nextDate);const name=r.productName||"可菱水濾心";
    ...
    console.log("濾心提醒完成 共"+rows.length+"筆 成功"+sent+"則");return{total:rows.length,sent};
  }catch(e){throw e}
}
async function runWarrantyReminders(){
  try{const rows=await dbGet("registrations","");let sent=0;
    for(const r of rows){const d=daysDiff(r.warrantyEnd);
    ...
    try{
      ...
      if(recent&&recent.length>0){
        ...
        if(eType==="message"&&ev.replyToken){
          ...
          const dLeft=daysDiff(rem.nextDate);
          await axios.post(LINE_REPLY,
            {replyToken:ev.replyToken,messages:[{type:"text",text:"✅ 濾心提醒設定完成！\n\n產品："+rem.productName+"\n到期日："+rem.nextDate+"（還有 "+dLeft+" 天）\n\n到期前我們會自動通知您 💧"}]},
            {headers:{"Content-Type":"application/json","Authorization":"Bearer "+token}}
          ).catch(()=>{});
        }
      }else{
        console.log("此userId無對應reminder："+wId.substring(0,10));
      }
    }catch(e){console.error("Webhook錯誤:",e.message)}
  }
});
