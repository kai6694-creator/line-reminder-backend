...
async function getDirectToken(){
  // 優先用 long-lived token（直接可用，不需要換 token）
  if(process.env.LINE_CHANNEL_ACCESS_TOKEN) return process.env.LINE_CHANNEL_ACCESS_TOKEN;
  // 備用：用 client_credentials 換 token
  return await getLineToken();
}
...
async function runWarrantyReminders(){try{const rows=await dbGet("registrations","");let sent=0;for(const r of rows){const d=daysDiff(r.warrantyEnd);if(d===30||d===7){try{const token=await getDirectToken();await axios.post(LINE_API,{to:r.userId,messages:[{type:"text",text:"📋【保固到期提醒】\n\n您的【"+r.productName+"】保固到期日："+r.warrantyEnd+"（還有 "+d+" 天）\n\n如有任何問題請聯繫我們！\n📞 02-2756-5899"}]},{headers:{"Content-Type":"application/json","Authorization":"Bearer "+token}});sent++;}catch(e){console.error("保固提醒失敗",e.response?.data)}}}return{total:rows.length,sent};}catch(e){throw e}}
...
app.get("/api/get-followers",async(req,res)=>{if(!checkAdmin(req,res))return;try{const token=await getDirectToken();const H={"Authorization":"Bearer "+token};const botR=await axios.get("https://api.line.me/v2/bot/info",{headers:H});const followersR=await axios.get("https://api.line.me/v2/bot/followers/ids",{headers:H});res.json({botInfo:botR.data,followers:followersR.data});}catch(e){res.status(500).json({error:e.response?.data||e.message})}});
